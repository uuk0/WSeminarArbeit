...
\begin{document}
    ...
    Wenn C negativ wird, können wir $C \rightarrow -C$ substituieren, und erhalten:

    \[
        = (A + B) * (-1) * C = (-1) * ((A + B) * C) = (-1) * (AB + AC) =
    \]

    Wir nehmen nun $A' = AC$ und $B' = BC$ und erhalten mit Substitution der Invesionsformel:
    ...
    Und nun nehmen wir $b_1$ und $b_2$ als Summanden:
    ...
    Und resubstituiert $C \rightarrow -C$ das gesuchte $AC + BC$

    Wenn A oder B $< 0$ ist, wir nehmen an, es sei A, substituieren wir $A \rightarrow -A$ und erhalten $-AC+BC$,
    und können wieder resubstituieren. Gleichermaßen wenn A und B negativ sind.
